...
Utils.buffToXYZAccel= function(buffer){
    ...
    pos = {                           //full scale set to 4g, therefore multiply time .002
        ...
        y: this.trueRound( this.twoscomp(((buffer[3] << 8) | buffer[2]) >> 4,12) * 0.002 -0.03, 2 ),  //add offset of -0.06 to y axis
        z: this.trueRound( this.twoscomp(((buffer[5] << 8) | buffer[4]) >> 4,12) * 0.002 -0.3 , 2 ) //add offset of -0.3 to z axis
    };
    return pos;
}
